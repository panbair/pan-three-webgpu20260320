/**
 * 量子流体模拟特效 - WebGPU 混合优化版 v2
 *
 * 使用 WebGPU Compute Shader 进行物理计算 + GPU 动态着色 + CPU 降频渲染
 * 性能优化：GPU 物理计算 + GPU 动态颜色 + 降频 CPU 渲染 + 多层微调
 *
 * 特性：
 * - 200000 个粒子（高性能）
 ...
 * - 降频 CPU 渲染（每 3 帧更新一次实例矩阵）
 * - 降频颜色计算（每 500 个粒子更新一次）
 * - 降低像素比（0.7）提升渲染性能
 * - 鼠标交互
 * - 赛博朋克霓虹配色
 * - 6 段电影级运镜
...
export const QuantumFluidTaichiEffectParams = {
  ...
  colorUpdateInterval: 500, // 颜色更新间隔（每500个粒子更新一次）
...
export const quantumFluidTaichiEffect = async (container: HTMLElement): Promise<() => void> => {
  ...
  async function init() {
    console.log('[QuantumFluidTaichi] 初始化 WebGPU 场景 v2（微调优化版）...')
    ...
    const neonColor4 = uniform(new THREE.Vector3(1.0, 0.6, 0.0)) // 金橙 0xff9900
          ...
          .select(
            ...
            segmentIndex
              ...
              .select(
                ...
                segmentIndex
                  .equal(float(2))
                  .select(neonColor2, segmentIndex.equal(float(3)).select(neonColor3, neonColor4))
              )
          )
          ...
          .select(
            ...
            segmentIndex
              ...
              .select(
                ...
                segmentIndex
                  .equal(float(2))
                  .select(neonColor3, segmentIndex.equal(float(3)).select(neonColor4, neonColor0))
              )
          )
    ...
    cinematicTimeline = gsap.timeline({
      ...
      repeat: 0, // 不重复
      onComplete: () => {
        console.log('[QuantumFluidTaichi] 运镜动画完成，开始清理特效')
        clearEffect()
      }
    })
    ...
    console.log('[QuantumFluidTaichi] 开始动画循环（混合优化模式 v2）')
    ...
    function clearEffect() {
      console.log('[QuantumFluidTaichi] 开始清理特效...')

      // 1. 淡出所有元素
      if (fluidMaterial) {
        gsap.to(fluidMaterial, {
          opacity: 0,
          duration: 1,
          ease: 'power2.out',
          onComplete: () => {
    ...
    function performCleanup() {
      console.log('[QuantumFluidTaichi] 执行清理流程...')
    ...
    console.log('[QuantumFluidTaichi] 初始化完成（混合优化模式 v2）')
  }
